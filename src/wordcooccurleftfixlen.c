...
int get_cooccurrence() {
  ...
  for (a = 1; a <= vocab_size; a++) {
    if ((lookup[a] = max_product / a) < vocab_size) lookup[a] += lookup[a-1]; // max_product <  max_n_row * vocab_size
    else lookup[a] = lookup[a-1] + vocab_size;  }  //default
  ...
  while (1) {
    ...
    if (htmp  != NULL) {
      //w2 = htmp->id;     // Target word (frequency rank)
        w2 = htmp->id + 1; // Target word (frequency rank)
    }else{w2=1;} //index is 1 -> n_vocab+2
	...
	//bigram_table[lookup[w1-1] + w2 - 2] += 1.0/((real)(j-k)); // Weight by inverse of distance between words
	  bigram_table[lookup[w1-1] + w2 - 2] += 1.0; 
